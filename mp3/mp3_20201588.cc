...
void buildHuffmanTree(const string& filename) {
    ...
    for (const auto& pair : hufftable) {
        // 문자를 이진 코드로 쓰기
        ofs.write(&pair.first, sizeof(pair.first));

        // 허프만 코드의 길이를 이진 코드로 쓰기
...
void loadAndDecode(string& filename) {
    ...
    while (true) {
        ...
        if (key == '\0') continue;  // 널 문자를 만나면 무시하고 다음 반복으로 진행
        if (key == '\x1c') break;  // 구분자를 만나면 허프만 테이블 읽기 종료
        ...
        for (int i = 0; i < length; i += 8) {
            ...
            ifs.read(&c, sizeof(c));
            bitset<8> bits(c);
            value += bits.to_string().substr(0, min(8, length - i));
        }
